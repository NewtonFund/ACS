...
/*
    RA  = acs_mem->TrackRA;
    Dec = acs_mem->TrackDec;

    if ( acs_mem->EnableCorr )
    {
        switch (acs_mem->CorrFrame )
            case ACS_FRAME_INSTRUMENT: 
              
               break;

            case ACS_FRAME_CELEST: 
               
               break;

            case ACS_FRAME_MOUNT:
               break;
...
//    if ( iauTaitt ( tai1, tai2, &tt1,  &tt2  ) ) return -1;
    ...
    if ( iauAtco13( RA, Dec,
                   ...
                   &acs_mem->DmdAzm, &acs_mem->DmdZD, &acs_mem->DmdRot, &acs_mem->DmdDec, &acs_mem->DmdRA, &acs_mem->DmdEO ) )
    { 
        return ACS_FAILURE;
    }
    else
    { 
        acs_debug( DBG3, "Demand: ZD=%.15f\t AZ=%.15f", acs_mem->DmdZD * DR2D, acs_mem->DmdAzm * DR2D );
    }
